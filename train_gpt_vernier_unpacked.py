...
def gptq_quantize_weight(w,H,clip_sigmas=3.,clip_range=63,block_size=128):
	...
	return Q[:,invperm],s
def gptq_vernier_quantize_weight(w,H,clip_sigmas=3.,clip_range=63,block_size=128,delta=.5/31.):
	# Vernier-Ladder: two interleaved int6 grids per row.
	#   grid_a values: level_a * s_a   (level in [-31,31])
	...
	for i1 in range(0,cols,block_size):
		...
		for j in range(i2-i1):
			...
			la=torch.clamp(torch.round(w_col/sf),-clip_range,clip_range);va=la*sf
			lb=torch.clamp(torch.round((w_col-half_a)/sf_b),-clip_range,clip_range);vb=lb*sf_b+half_a
			ea=(w_col-va).abs();eb=(w_col-vb).abs();sel=(eb<ea)
			level=torch.where(sel,lb,la);v=torch.where(sel,vb,va)
			Q_v[:,i1+j]=level.to(torch.int8);Sel_v[:,i1+j]=sel.to(torch.uint8)
			err=(w_col-v)/d;Err[:,j]=err;sse_v+=(w_col-v).double().pow(2)
			W_block[:,j:]-=err.unsqueeze(1)*Hinv_block[j,j:].unsqueeze(0)
		if i2<cols:W_work[:,i2:]-=Err@Hinv[i1:i2,i2:]
	# Plain int6 pass (independent run for fair per-row comparison).
	Q_p=torch.zeros(rows,cols,dtype=torch.int8);W_work=W_perm.clone();sse_p=torch.zeros(rows,dtype=torch.float64)
	for i1 in range(0,cols,block_size):
		i2=min(i1+block_size,cols);W_block=W_work[:,i1:i2].clone();Hinv_block=Hinv[i1:i2,i1:i2];Err=torch.zeros(rows,i2-i1)
		for j in range(i2-i1):
			w_col=W_block[:,j];d=Hinv_block[j,j]
			q_col=torch.clamp(torch.round(w_col/sf),-clip_range,clip_range);v=q_col*sf
			Q_p[:,i1+j]=q_col.to(torch.int8);err=(w_col-v)/d;Err[:,j]=err;sse_p+=(w_col-v).double().pow(2)
			W_block[:,j:]-=err.unsqueeze(1)*Hinv_block[j,j:].unsqueeze(0)
		if i2<cols:W_work[:,i2:]-=Err@Hinv[i1:i2,i2:]
...
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	for(name,tensor)in state_dict.items():
		...
		cat=classify_param(name);cs=h.embed_clip_sigmas if'tok_emb'in name else h.matrix_clip_sigmas;bits=h.embed_bits if'tok_emb'in name else h.matrix_bits;clip_range=2**(bits-1)-1
...
def dequantize_mixed(result,meta,template_sd,delta=.5/31.):
	...
	for(name,orig)in template_sd.items():
		...
		if'passthrough'in info:
			...
			out[name]=t;continue
		if'gptq_vernier'in info:
			q=result[name+'.q'].float();sel=result[name+'.sel'].float();s_a=result[name+'.scale'].float().view(q.shape[0],1);use_vernier=result[name+'.uv'].view(q.shape[0],1);s_b=s_a*(1.+float(delta));half_a=.5*s_a
			# Per-weight Vernier dequant; rows with use_vernier=False fall back to plain int6.
			vernier_w=torch.where(sel>.5,q*s_b+half_a,q*s_a);plain_w=q*s_a
			out[name]=torch.where(use_vernier,vernier_w,plain_w).to(orig_dtype)
			continue
...
def deserialize(h,device):
	...
	quant_state=torch.load(io.BytesIO(_decompress(quant_blob_disk,h.compressor)),map_location='cpu');deq_state=dequantize_mixed(quant_state['w'],quant_state['m'],sd_cpu,delta=h.vernier_delta);eval_model.load_state_dict(deq_state,strict=True);return eval_model
