#!/usr/bin/env python3
"""GDN Hybrid Full Training Script — 7000 steps with all production features.

Features beyond Phase 1 screening (train_gdn.py):
  - EMA weight averaging (decay 0.997)
  - SWA (stochastic weight averaging) during late warmdown
  - Late QAT (int6 STE in CastedLinear forward during warmdown)
  - Mixed int6/int8 quantization with percentile search
  - brotli-11 compression for artifact
  - Coprime shard ordering via SHARD_ORDER_FILE
  - XSA-all sliding window eval on quantized artifact
  - Roundtrip validation (load quantized, eval, report exact BPB)

Environment variables (key additions vs Phase 1):
    EMA_DECAY: EMA decay rate (default 0.997)
    SWA_ENABLED: 1|0 (default 1)
    SWA_EVERY: SWA collection interval (default 50)
    LATE_QAT_THRESHOLD: LR scale below which QAT activates (default 0.15)
    SHARD_ORDER_FILE: path to file with one shard path per line (coprime ordering)
    MUON_MOMENTUM_WARMUP_START: starting momentum for warmup (default 0.85)
    MUON_MOMENTUM_WARMUP_STEPS: steps to ramp momentum (default 500)
"""
...
class Hyperparameters:
    ...
    warmdown_iters = int(os.environ.get("WARMDOWN_ITERS", 2100))  # 30% of 7k
...
def generate_coprime_shard_order(shard_files: list, seed: int = 42) -> list:
    """Generate a coprime-stepping shard order for better data mixing.

    Instead of sequential 0,1,2,...,79, uses stride = coprime(N) to visit
    all shards in a maximally-spread order. This ensures each training epoch
    sees data from diverse shards rather than correlated sequential ones.
    """
    n = len(shard_files)
    if n <= 1:
        return shard_files

    # Find a coprime stride that's roughly n/phi (golden ratio)
...
def eval_val_ttt_gdn(
    ...
    """Legal score-first TTT for GDN: score each chunk with sliding windows,
    then SGD-train on already-scored tokens. Every token scored BEFORE any update."""
    ...
    log0(f"ttt_gdn:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
    ...
    # Freeze first N blocks (early layers learn general features, keep stable)
    ...
    log0(f"ttt_gdn:params unfrozen={sum(p.numel() for p in ttt_params)} "
    ...
    for ci in range(num_chunks):
        ...
        # Skip training on last chunk (no future windows benefit from it)
    ...
    log0(f"ttt_gdn:done val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} "
...
# Control tensor patterns — kept at full precision during quantization
CONTROL_PATTERNS = (
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    """Int6 quantization with percentile search for optimal clipping."""
    t32 = t.float()
    if t32.ndim == 2:
    ...
    # 1D: simple per-tensor quantization
...
def quantize_int8_per_row(t: Tensor) -> tuple[Tensor, Tensor]:
    """Int8 quantization with percentile clipping."""
    t32 = t.float()
    clip_q = 0.9999984
    if t32.ndim == 2:
...
def mixed_quantize(state_dict: dict[str, Tensor], hessians: dict[str, Tensor] | None = None) -> tuple[dict[str, Tensor], dict[str, object]]:
    """Mixed int6 (large weights) / int8 (small weights) quantization."""
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    for name, tensor in state_dict.items():
        t = tensor.detach().cpu().contiguous()
        # Control tensors: keep fp16 passthrough
        ...
        if t.numel() <= 65536:
            ...
            meta[name] = "passthrough"
            continue
        # Large 2D weights: int6 (6-bit quantization for better compression)
...
def dequantize_mixed(result: dict[str, Tensor], meta: dict[str, object],
    ...
    """Dequantize mixed int6/int8 back to float."""
    out: dict[str, Tensor] = {}
...
def save_full_checkpoint(model, step, val_bpb, ckpt_dir, arch_name, seed,
    ...
    """Save complete training state for chained job resume."""
    base = model.module if hasattr(model, 'module') else model
    ckpt = {
        "step": step, "val_bpb": val_bpb,
        "arch_name": arch_name, "seed": seed,
...
def _find_latest_full_ckpt(ckpt_dir):
    """Find the latest full_ckpt_step*.pt file in ckpt_dir by step number."""
...
def main():
    ...
    log0(f"=== GDN Hybrid 7k Full Training ===")
    log0(f"Arch: {config['arch_name']} (ARCH_MODE={args.arch_mode})")
    log0(f"Seed: {args.seed}, Steps: {args.iterations}, Warmdown: {args.warmdown_iters}")
    ...
    start_step = 0
    resume_state = None  # holds full checkpoint data for deferred restore
    ...
    if resume_ckpt_path and os.path.exists(resume_ckpt_path):
        ...
        # Keep full checkpoint for deferred optimizer/EMA/SWA restore
    ...
    # Deferred restore: optimizer states (must happen after optimizer creation)
    ...
    # Data loader — coprime shard ordering
    shard_order_file = os.environ.get("SHARD_ORDER_FILE", "")
    ...
    # LR schedule with warmdown (cosine)
    ...
    # Deferred restore: EMA, SWA, QAT, RNG, stream state
    if resume_state is not None:
        # EMA
    ...
    # ─── Training Loop ───────────────────────────────────────────────────
    # Clear stale chain marker from previous segment (if any)
    ...
    log0(f"\n{'='*80}")
    log0(f"Starting training: {args.iterations} steps (from step {start_step})")
    log0(f"{'='*80}\n")
    ...
    step = start_step  # ensure step is defined even if loop doesn't execute
    ...
    for step in range(start_step + 1, args.iterations + 1):
        ...
        # Late QAT: activate int6 STE only during warmdown (not warmup!)
        warmdown_start = args.iterations - args.warmdown_iters
        ...
        # Auto-save for chained job support
        if args.auto_save_seconds > 0:
            ...
            if elapsed > args.auto_save_seconds:
                ...
                if master_process:
                    ...
                    # Write chain resume marker
                    marker_path = os.path.join(args.ckpt_dir, f"CHAIN_RESUME_FROM_seed{args.seed}")
                    with open(marker_path, "w") as f:
                        f.write(ckpt_path + "\n")
                    log0(f"  Full checkpoint saved: {ckpt_path}")
                    log0(f"  Chain resume marker: {marker_path}")
                break  # exit training loop cleanly

    # ─── Check if we exited due to auto-save vs normal completion ────────
    ...
    # Check for total_iterations completion
    ...
    # Build fresh eval model (no DDP wrapping needed)
    ...
    log0(f"  Training: {args.iterations} steps, {elapsed_total:.0f}s")
    log0(f"  EMA BPB (fp32):     {val_bpb_ema:.6f}")
