#!/usr/bin/env python3
"""Parameter Golf evaluator — Lepton GPU job submission and result capture.

Contract:
  --threshold X --node-group NG [--result-path PATH] → stdout JSON + exit 0/1
  --3seed --commit SHA --node-group NG1 --node-group NG2 → stdout JSON
  --preflight --commit SHA --node-group NG → exit 0/1
  --list / --stop JOB_ID / --stop-all

This tool is DUMB — it never decides retries, pivots, or next actions.
Those decisions belong to the autophd skill (the Claude agent).
"""
...
FORWARDED_JOB_ENV_KEYS = (
    ...
    "COMPRESSOR",
    "QK_GAIN_INIT",
    # PR #1855 9-hp greedy stack overrides — also baked into train_gpt.py defaults
    # in this branch but kept forwardable so reviewers can still A/B-test:
    "MLP_CLIP_SIGMAS",
    "WARMDOWN_FRAC",
    "BETA2",
    "TTT_BETA2",
    ...
    # CaseOps tokenizer + #1953 mechanism levers (long-context 2560, no_qv mask,
    # local LoRA-TTT LR multiplier, AWQ-lite, asymmetric logit rescale, LQER):
    "CASEOPS_ENABLED",
    "EVAL_SEQ_LEN",
    "TTT_EVAL_SEQ_LEN",
    "TTT_MASK",
    ...
    "LQER_ASYM_ENABLED",
    "LQER_ASYM_GROUP",
    # Additional #1855 / #1953 stack feature flags + training hyperparams:
...
"$PYBIN" -m torch.distributed.run --nproc_per_node=${{PGOLF_NPROC:-8}} train_gpt.py
RC=$?
...
def _has_final_results_content(content):
    ...
    # TTT (LoRA / phased / global) runs after quantized roundtrip/sliding and is the real final metric.
    if "ttt_enabled: True" in content:
        return (
            "quantized_ttt_lora" in content
            or "quantized_ttt_phased" in content
            or "quantized_ttt_global" in content
            or "ttt_sliding:done" in content
        )
...
def _result_file_path(explicit_path=None) -> str:
    if explicit_path:
    ...
    # AUTOPHD_NOTIFY_TMUX_PANE is read elsewhere (harmless no-op now); keep reference.
    _ = os.environ.get("AUTOPHD_NOTIFY_TMUX_PANE")
...
def main():
    ...
    if args.result_path:
        os.environ["AUTORESEARCH_RESULT_FILE"] = args.result_path
    queue_failfast_seconds = int(os.environ.get("PGOLF_QUEUE_FAILFAST_SECONDS", "300"))
    queue_failfast_reasons = {"InsufficientQuota", "InsufficientResources"}
    ...
    # 1. Detect branch (push only if commit not pre-supplied by caller)
    ...
    if args.commit:
        # Worker has already committed + pushed; skip push, use supplied SHA.
        commit = args.commit
    else:
